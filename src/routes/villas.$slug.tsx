...
export const Route = createFileRoute("/villas/$slug")({
  ...
  head: ({ loaderData }) => {
    ...
    if (!v) return { meta: [{ title: "Villa — Ario Stays" }] };
    return {
      meta: [
        { title: `${v.name} — Ario Stays` },
        { name: "description", content: v.tagline },
        { property: "og:title", content: `${v.name} — Ario Stays` },
        { property: "og:description", content: v.tagline },
  ...
  notFoundComponent: () => (
    <Layout>
      <section className="container-editorial">
        ...
        <Link to="/villas" className="mt-6 inline-block text-sm uppercase tracking-[0.18em] text-navy underline">
          Back to all villas
        </Link>
      </section>
    </Layout>
  ...
  return (
    <Layout>
      <section className="container-editorial">
        <Link
          to="/villas"
          className="inline-flex items-center gap-2 text-[11px] uppercase tracking-[0.25em] text-muted-foreground hover:text-navy"
        >
          ← Back to all villas
        </Link>
        ...
        <div className="mt-6 grid gap-3 md:grid-cols-12 md:gap-4">
          <div className="md:col-span-8">
            <div className="aspect-[4/3] overflow-hidden rounded-3xl bg-secondary md:aspect-[16/10]">
              <img src={villa.images[active]} alt={villa.name} className="h-full w-full object-cover" />
            </div>
          </div>
        ...
        <div className="mt-12 grid gap-12 md:grid-cols-12">
          <div className="md:col-span-7">
            ...
            <div className="mt-8 flex flex-wrap gap-6 border-y border-border py-5 text-sm text-muted-foreground">
              ...
              <div>
                ...
                <p className="mt-1 font-serif text-xl text-navy">₹{villa.price.toLocaleString("en-IN")} <span className="text-xs text-muted-foreground">/ night</span></p>
              </div>
            </div>

            <p className="mt-8 text-base leading-relaxed text-muted-foreground">{villa.description}</p>
            ...
            <div className="mt-12">
              ...
              <a href={villa.mapLink} target="_blank" rel="noopener noreferrer" className="mt-3 inline-block text-xs uppercase tracking-[0.18em] text-navy underline">
                Open in Google Maps →
              </a>
            </div>
          </div>
          ...
          <aside className="md:col-span-5">
            <div className="sticky top-28 rounded-3xl border border-border bg-card p-7 shadow-[var(--shadow-soft)]">
              <p className="text-[11px] uppercase tracking-[0.25em] text-muted-foreground">Starting from</p>
              ...
              <p className="mt-2 text-xs text-muted-foreground">Taxes & service charges may apply</p>
        ...
        <div className="mt-32 border-t border-border pt-16">
          <div className="flex items-end justify-between gap-6">
            ...
            <Link to="/villas" className="hidden text-xs uppercase tracking-[0.18em] text-navy underline md:inline">
              View all →
            </Link>
          </div>
